...
@ManagedBean
@ViewScoped
public class DepressionScaleImportBean implements Serializable {
    ...
    public void upload() throws IOException {
        ...
        while (linha != null) {
            ...
            String question10 = "";
            try {
                question10 = linhaq[11].trim();
            }catch(Exception e){
                
            }

            DepressionScale stressScale = new DepressionScale(uid, type, question1,
    ...
    private void distributeDevices() {
        ...
        List<DepressionScale> stressScale = fetchStressScale();
        Integer quantity = stressScale.size() / auxList.size();
        Integer i = 0;
        Integer max = 0;
        EntityManager em = JPAUtil.getEntityManager();
        em.getTransaction().begin();
        for (DepressionScale stressScaleObj : stressScale) {
            try {
    ...
    private List<DepressionScale> fetchStressScale() {
        EntityManager em = JPAUtil.getEntityManager();
        Query query = em.createQuery("SELECT i FROM DepressionScale i");
        try {
